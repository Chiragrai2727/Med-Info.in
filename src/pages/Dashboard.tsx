...
export const Dashboard: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-24 sm:pt-32 pb-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto space-y-8">
        
        {/* Header */}
        <div>
          <h1 className="text-3xl sm:text-4xl font-bold text-gray-900">{t('myDashboard')}</h1>
          <p className="text-gray-500 mt-2 text-sm sm:text-base">{t('dashboardDesc')}</p>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="p-6 sm:p-8 flex flex-col sm:flex-row items-center gap-6">
            {profile.photoURL ? (
              <img src={profile.photoURL} alt="Profile" className="w-20 h-20 sm:w-24 sm:h-24 rounded-full border-4 border-gray-50 shadow-md" referrerPolicy="no-referrer" />
            ) : (
            ...
            <div className="text-center sm:text-left">
              <h2 className="text-2xl sm:text-3xl font-black text-gray-900">{profile.displayName || t('user')}</h2>
              <p className="text-gray-500">{profile.email}</p>
              <div className="mt-4 flex flex-wrap justify-center sm:justify-start gap-2 sm:gap-3">
                <span className={`px-4 py-1.5 text-xs font-bold rounded-full capitalize ${
