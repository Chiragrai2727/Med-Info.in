...
export const Timetable: React.FC = () => {
  ...
  const handleAddSchedule = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setIsAdding(false);
      setNewSchedule({ medicineName: '', dosage: '', time: '08:00', days: [...DAYS], whatsappEnabled: true });
  ...
  const handleWhatsAppReminder = (schedule: Schedule) => {
    const time = schedule.time;
    const medicine = schedule.medicineName;
    const dosage = schedule.dosage;
    const message = `💊 *Medication Reminder from Aethelcare*\n\nIt's time to take your medicine:\n*Medicine:* ${medicine}\n*Dosage:* ${dosage}\n*Scheduled Time:* ${time}\n\nStay healthy! https://aethelcare.xyz/timetable`;
    
    // In a real app we'd use the user's phone, for now we let them choose the contact
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-6">
          <div>
            ...
            {isPremium ? (
              <div className="bg-[#25D366]/10 border border-[#25D366]/30 text-[#075E54] px-4 py-2 mt-4 rounded-xl flex items-center justify-between gap-4 max-w-sm">
                <div className="flex items-center gap-2">
                  <div className="w-2 h-2 bg-[#25D366] rounded-full animate-pulse" />
                  <span className="font-bold text-xs uppercase tracking-widest">WhatsApp Reminders Active</span>
                </div>
                <div className="text-[10px] font-black opacity-60">PREMIUM</div>
              </div>
            ) : (
              <div className="bg-[#25D366]/10 border border-[#25D366]/30 text-[#075E54] px-4 py-2 mt-4 rounded-xl flex items-center justify-between gap-4 max-w-sm">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-bold text-xs uppercase tracking-widest">Connect WhatsApp Reminders</span>
                </div>
                <button onClick={() => setShowPaywall(true)} className="text-[#128C7E] font-black text-[10px] uppercase tracking-widest hover:underline">
                  Upgrade
                </button>
              </div>
            )}
                      ...
                      >
                        <div className="flex items-center gap-8">
                         ...
                         <div className="flex items-center justify-between md:justify-end gap-6 text-sm">
                           {isPremium && (
                             <button
                               onClick={() => handleWhatsAppReminder(schedule)}
                               className="p-4 bg-[#25D366]/10 text-[#075E54] rounded-2xl hover:bg-[#25D366] hover:text-white transition-all group scale-100 hover:scale-105 active:scale-95"
